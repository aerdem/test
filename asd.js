...
$(document).ready(function() {
    ...
    $("#signup_form").submit(function(event) {
        ...
        setTimeout(function() {
            ...
            if ($("#signup_recaptcha").length > 0) {
                post_variables['g_recaptcha_response'] = $("#signup_recaptcha").find('#g-recaptcha-response').val() || grecaptcha.getResponse();
            } else if ($("#captchasnet_signup_captcha").length > 0) {
    ...
    $("#main_to_adv_button").click(function(event) {
        var transfer_amt = $("#main_to_adv_amount").val();
    ...
    $("#edit_ad_params_button").click(function() {
        var daily_budget = $('#ad_details_popup_daily_budget').val();
        var total_budget = $('#ad_details_popup_total_budget').val();
        var campaign_name = $('#ad_details_popup_campaign_name').val();
        var ad_id = $('#ad_details_popup_ad_id').val();
        var freq_cap = $('#ad_details_popup_freq_cap').val();
        var adv_bit = 0;
        var adv_doge = 0;
    ...
    $("#bcc_withdrawal_button").click(function(event) {
        $("#bcc_withdrawal_button").attr("disabled", true);
        var posting = $.post('/', {
            op: 'withdraw_bcc',
            withdraw_address: $("#bcc_withdraw_btc_add").val(),
            tfa_code: $("#bcc_tfa_code").val()
        });
        posting.done(function(data) {
            var result = data.split(":");
            if (result[0] == "s") {
                $('#bcc_withdraw_btc_add').val('');
            }
            DisplaySEMessage(result[0], result[1]);
            $("#bcc_withdrawal_button").attr("disabled", false);
        });
    });
    ...
    $("#free_play_form_button").click(function(event) {
        ...
        } else if ($("#captchasnet_free_play_captcha").length > 0) {
...
function GenerateCaptchasNetCaptcha(parent_div, backup) {
    ...
    $.get('/cgi-bin/api.pl?op=generate_captchasnet&f=' + fingerprint, function(data) {
        if (data.length < 100) {
            var image_url = "//captchas.freebitco.in/cgi-bin/captcha_generator?client=freebitcoin&random=" + data; /*if (backup == 1){image_url = "//image.captchas.net/?client=freebitcoin&random="+data;}*/
